...
# Assuming your dataset script is named meld_dataset.py in the same directory
try:
...
class VedioEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.backbone = vision_models.video.r3d_18(weights='R3D_18_Weights.DEFAULT')
...
class MultimodalSentimentModel(nn.Module):
    def __init__(self):
        ...
        self.video_encoder = VedioEncoder()
...
class MultimodalTrainer:
    ...
    def log_metrics(self, losses, metrics=None, phase='train'):
        if phase == 'train':
            ...
            self.writer.add_scalar('loss/total/train', losses['total'], self.global_step)
        else:
    ...
    def train_epoch(self):
        ...
        for batch in self.train_loader:
            ...
            emotional_labels = batch['emotion'].to(device)
            sentiment_labels = batch['sentiment'].to(device)
            
            self.optimizer.zero_grad()
            outputs = self.model(text_input, video_frames, audio_features)
            
            emotion_loss = self.emotion_criterion(outputs['emotions'], emotional_labels)
            ...
            self.log_metrics({'total': total_loss.item(), 'emotion': emotion_loss.item(), 'sentiment': sentiment_loss.item()})
            self.global_step += 1
    ...
    def evaluate(self, data_loader, phase='val'):
        ...
        with torch.inference_mode():
            for batch in data_loader:
                ...
                all_emotion_labels.extend(emotional_labels.cpu().numpy())
        ...
        metrics = {
            ...
            'emotional_accuracy': accuracy_score(all_emotion_labels, all_emotion_preds),
        ...
        self.log_metrics(avg_loss, metrics, phase=phase)
        if phase == 'val':
...
if __name__ == "__main__":
    ...
    for i, prob in enumerate(emotional_probs):
        print(f"{emotion_list[i]}: {prob.item():.2f}")
        
    print("\n--- Sentiment Probabilities ---")
    for i, prob in enumerate(sentiment_probs):
        print(f"{sentiment_list[i]}: {prob.item():.2f}")
        
    print("\nPredictions complete for utterance.")